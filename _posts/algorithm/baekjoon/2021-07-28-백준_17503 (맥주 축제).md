...
1. 맥주를 도수 레벨 순으로 정렬
2. 우선순위 큐에 맥주의 선호도 값을 추가 (이때 선호도를 누적해서 합산)
3. 우선순위 큐의 사이즈가 맥주의 갯수(n)과 같으면서 선호도의 총 합이 m 이상인 경우 프로그램 종료
...
public class 한규호_17503 {
  ...
  public static void main(String[] args) throws IOException {
    ...
    Arrays.sort(beerInfoArr,
        Comparator.comparingInt(item -> item[alcoholIdx])); // 맥주의 도수 레벨 순으로 배열 정렬
    ...
    for (int i = 0; i < k; i++) {
      ...
      if (pq.size() > n) { // pq의 갯수가 맥주의 갯수(n) 보다 크면?
        int minPreference = pq.peek() == null ? 0
            : pq.poll(); // 선호도가 가장 작은 값 (= pq에서 우선순위가 가장 높은 값) 을 우선순위 큐에서 제거한 후 총 합에서 차감
